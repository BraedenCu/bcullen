...
fn serve_static_file(path: &Path, request: &HttpRequest) -> RouteResult {
    ...
    let body = match fs::read(path) {
        Ok(b) => b,
        Err(_) => return RouteResult::Response(HttpResponse::internal_error("Failed to read file")),
    };
    ...
    for line in content.lines() {
        ...
        if let Some((key, value)) = trimmed.split_once(char::is_whitespace) {
            ...
            match key {
                ...
                "User" => {
                    stored_user = BASE64.decode(value).ok().and_then(|b| String::from_utf8(b).ok());
                }
                "Password" => {
                    stored_password =
                        BASE64.decode(value).ok().and_then(|b| String::from_utf8(b).ok());
                }
                _ => {}
            }
        }
