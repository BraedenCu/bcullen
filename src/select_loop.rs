...
/// A single event loop handling multiple connections via poll.
fn event_loop(
    id: usize,
    ...
    loop {
        // Check for new connections from the accept thread (non-blocking)
        loop {
            match new_conn_rx.try_recv() {
                ...
                Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                    // Accept thread is done — drain remaining connections
        ...
        // If no connections and channel disconnected, exit
        if connections.is_empty() && new_conn_rx.try_recv().is_err() {
            // Double-check: if the channel is disconnected and no connections, exit
            if state.shutdown.load(Ordering::Relaxed) {
                return;
            }
        }
                    ...
                    ConnState::ReadingRequest => {
                        if handle_read(conn, state, &poll) {
                            if conn.state == ConnState::Done {
                                ...
                                // Transition to write: reregister for WRITABLE
                                poll.registry()
                                    .reregister(
                                        &mut conn.stream,
                                        token,
                    ...
                    ConnState::WritingResponse => {
                        if handle_write(conn, &poll) {
                            if conn.state == ConnState::Done {
                                ...
                                // Keep-alive: re-register for read
                                poll.registry()
                                    .reregister(
                                        &mut conn.stream,
                                        token,
